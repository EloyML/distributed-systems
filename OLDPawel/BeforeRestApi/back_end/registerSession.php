 ...
 */
    ...
    if(require_once("Calendar/_dbCon.php")) {
        ...
        if ($stmt->execute()) {
            ...
            if ($result->num_rows >= 0) {
                ...
                safe_redirect("https://localhost/appdev/CalendarPage.php", false);
            } else {
                $connection->query("DELETE FROM USERS WHERE Email = '" . $_SESSION["email"] . "'");
                session_destroy();
                $connection->close();
                safe_redirect("https://localhost/Cloud/RegisterPage.php", false);
            }
        } else {
            $stmt->close();
            $connection->query("DELETE FROM USERS WHERE Email = '" . $_SESSION["email"] . "'");
